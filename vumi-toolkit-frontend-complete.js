(async function() {
    ...
    // আপনার Google Apps Script API Link (নতুন deployment URL দিয়ে আপডেট করুন)
    const API_URL = "https://script.google.com/macros/s/AKfycbxx18GYOdS-eVubQjpgIoqFK1IUWCvm03sLtsIfD46eSAJ6t02UFG1j6F2XoxfAxMctwQ/exec";
    ...
    // ✅ নতুন: Server থেকে Device ID নিন (সব domain এ same)
    async function getOrCreateDeviceId() {
        let userId = savedAuth ? atob(savedAuth) : "guest";
        
        try {
            // Server থেকে Device ID নিন (সব domain এ same)
            let response = await fetch(`${API_URL}?action=getDeviceId&userid=${encodeURIComponent(userId)}&t=${Date.now()}`);
        ...
        // Fallback: Local ID তৈরি করুন (যদি server down থাকে)
        let localId = null;
        if (typeof chrome !== 'undefined' && chrome.storage) {
            let data = await chrome.storage.local.get('vmt_device_id');
            localId = data.vmt_device_id;
            if (!localId) {
                localId = 'VMT-' + Math.random().toString(36).substr(2, 9) + '-' + Date.now();
    ...
    // Session check interval - 5 মিনিটে একবার
    setInterval(async () => {
        let exp = null;
        if (typeof chrome !== 'undefined' && chrome.storage) {
