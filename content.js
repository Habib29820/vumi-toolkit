(async function() {
    console.log("🚀 VUMI PRO: Script Initialization Started...");
    try {
        // পুরনো কোনো প্যানেল থাকলে ক্লিয়ার করবে
        let oldTk = document.getElementById('vumi-master-toolkit');
        let oldPo = document.getElementById('vmt-pass-overlay');
        let old404 = document.getElementById('vmt-404-overlay');
        if (oldTk) oldTk.remove(); if (oldPo) oldPo.remove(); if (old404) old404.remove();
        ...
        let deviceId = ""; let savedAuth = ""; let tFlag = ""; let sessionExpiry = "";

        // 🔴 1. RANDOM UNIQUE DEVICE ID (To prevent collision)
        ...
        function clearAuthData() {
            localStorage.removeItem('vmt_auth_hash'); localStorage.removeItem('vmt_t_flag'); localStorage.removeItem('vmt_session_expiry');
        }
            ...
            overlay.innerHTML = `
                <div style="text-align:center; width: 100%; max-width: 800px; padding: 40px 20px;">
                    ...
                    <div style="margin-top:-50px;">
                        ...
                        <button id="vmt-404-btn" style="color: #fff; padding: 12px 30px; background: #39ac31; border:none; border-radius: 5px; font-size: 16px; font-weight: bold; cursor: pointer; outline: none; box-shadow: 0 4px 6px rgba(57,172,49,0.3);">Go to Home / Reload</button>
                    </div>
                </div>
            `;
        ...
        async function checkLiveStatus() {
            try {
                ...
                if (data.success === false) {
                    ...
                    saveAuthData(savedAuth, 'engaged'); tFlag = 'engaged';
                } else if (data.success === true) {
                    saveAuthData(savedAuth, ''); tFlag = '';
                }
        ...
        setInterval(async () => {
            ...
            if (savedAuth && exp && Date.now() > parseInt(exp)) {
                clearAuthData(); window.location.reload(); 
        ...
        // ==========================================
        // 🔴 2. GLOBAL TOOLS DEFINITION (SUPER FAST) 🔴
        // ==========================================
        window._vmtTools = {
            t1: function() {
                ...
                inp.onchange = e => {
                    ...
                    r.onload = async function(ev) {
                        ...
                        for (let i = 0; i < es.length; i++) {
                            ...
                            await new Promise(r => setTimeout(r, 800)); // Was 2500
                        }
                        if (dgs.length > 0) {
            ...
            t5: function() {
                ...
                i.onchange = e => {
                    ...
                    r.onload = async function(ev) {
                        ...
                        // ⚡ SUPER FAST TOTTHO 2.0 ENTRY
                        for (let d of es) {
                            let b = Array.from(document.querySelectorAll('button')).find(x => x.innerText.includes('লিখে এন্ট্রি')); if (b) b.click(); await new Promise(r => setTimeout(r, 600)); // was 1800
                            ...
                            await new Promise(r => setTimeout(r, 300)); // was 1000
                            let bA = Array.from(document.querySelectorAll('button')).find(x => x.innerText.trim() === 'এন্ট্রি করুন'); if (bA) bA.click(); await new Promise(r => setTimeout(r, 800)); // was 2200
                            ...
                            await new Promise(r => setTimeout(r, 500)) // was 1200
                        }
            ...
            t8: async function() {
                ...
                for (let k = start; k <= end; k++) {
                    ...
                    try {
                        ...
                        searchBox.style.backgroundColor = '#fef08a';
                        searchBox.style.border = '3px solid #eab308';
                        ...
                        if (!searchBtn) {
                            searchBtn = allBtns.find(b => { let svg = b.querySelector('svg'); return (svg && !b.innerText && !b.innerHTML.includes('refresh') && !b.innerHTML.includes('close')); });
                        }
                        ...
                        rows.forEach(row => {
                            ...
                            if (cols.length > 1) {
                                let nameDetails = cols[0].innerText.trim(); let symbols = cols[1].innerText.trim();
                                if (nameDetails && nameDetails !== '১' && !nameDetails.includes('মালিক, অকৃষি')) { finalOutput.push(nameDetails + '    ' + symbols); }
                            }
                        });
                        ...
                        if (resultText !== '') {
                            ...
                            let khatianMatch = bodyText.match(/খতি[য়য়]ান\s*নং[^\d০-৯]*([০-৯0-9]+)/);
        ...
        t11: async function(upazilaStr, FCStr) {
            ...
            await (async function ownerPart(upazila, FC) {
                ...
                async function setId(id, v, tries = 3, gap = 200) {
                ...
                async function selectMouza() {
                  ...
                  if (input) input.focus(); if (popBtn) popBtn.click(); await sleep(600); // was 1300
                ...
                await setById("DIVISION_NAME", "ঢাকা"); await setById("DISTRICT_NAME", "নারায়ণগঞ্জ"); await setById("UPAZILA_NAME", upazila); await sleep(500); await selectMouza(); await sleep(400);
                await setById("NAMJARI_CASE_NO", mamlaNo); if (mamlaDate) setByLabel("মামলার তারিখ", bn2en(mamlaDate)); clickRadio("HAS_DHARA", "না"); await setById("KHATIAN_NO", khatianNo);
                for (let i = 0; i < owners.length; i++) {
                  const o = owners[i]; clickRadio("IDENTITY_TYPE", "নাই"); await sleep(200); await setById("OWNER_NAME", o.name); if (o.gName) clickRadio("GUARDIAN_TYPE", o.gType);
                  await setById("GUARDIAN", o.gName); await setById("MOTHER_NAME", o.mother); await setById("OWNER_AREA", ownerShare); await setById("OWNER_GROUP", o.group); await setById("OWNER_ADDRESS", address);
                  await sleep(200); clickButton("মালিক যোগ করুন"); await sleep(600); // was 1600
                } clickButton("মালিক এন্ট্রি সমাপ্ত করুন"); await sleep(600); // was 1500
            })(upazila, FC);

            // ⚡ SUPER FAST DAG ENTRY
            await (async function dagPart() {
                ...
                async function setById(id, value) {
                  ...
                  for (let t = 0; t < 3; t++) { reactSet(el, value); await sleep(150); if (norm(el.value) === norm(value)) break; } // was 350
                }
                async function setRemarks(value) {
                  ...
                  if (el) { for (let t = 0; t < 3; t++) { reactSet(el, value); await sleep(150); if (norm(el.value) === norm(value)) break; } }
                }
                ...
                async function pickAuto(inputEl, optionText) {
                  if (!inputEl) return; inputEl.focus(); reactSet(inputEl, optionText); await sleep(300); // was 700
                  let opts = Array.from(document.querySelectorAll('li[role="option"]'));
                  if (!opts.length) { const root = inputEl.closest(".MuiAutocomplete-root"); const pop = root && root.querySelector(".MuiAutocomplete-popupIndicator"); if (pop) pop.click(); await sleep(400); opts = Array.from(document.querySelectorAll('li[role="option"]')); }
                ...
                async function pickSelect(labelStart, wantSubstr) {
                  ...
                  if (!trigger) return; trigger.focus(); trigger.dispatchEvent(new MouseEvent("mousedown", { bubbles: true })); trigger.click(); await sleep(300); // was 800
                  ...
                  if (t) { t.dispatchEvent(new MouseEvent("mousedown", { bubbles: true })); t.click(); await sleep(200); } else { document.body.click(); }
                }
                ...
                const dags = splitField(grab("দাগ নং") || grab("দাগ")); const classToks = splitField(grab("জমির শ্রেণি") || grab("শ্রেণি") || grab("শ্রেণী")); const totalToks = splitField(grab("দাগে মোট জমির পরিমান") || grab("মোট জমির পরিমান") || grab("মোট")); const atraToks = splitField(grab("অত্র خতিয়ানের জমির পরিমান") || grab("অংশ অনুযায়ী জমির পরিমান") || grab("অত্র"));
                ...
                for (let i = 0; i < dags.length; i++) {
                  ...
                  if (i === maxIdx) await setRemarks(montobbo); await sleep(200); clickButton("দাগ যোগ করুন"); await sleep(600); // was 1600
                } await sleep(400); clickButton("দাগ এন্ট্রি সমাপ্ত করুন"); alert("✅ ম্যানুয়াল খতিয়ান এন্ট্রি সফলভাবে সম্পন্ন হয়েছে!");
            })();
        }
    };

    // 🔴 6. MAIN EXECUTION & UI LOADER
    function loadToolkit() {
        ...
        <div class="vmt-body" style="min-height: 260px;">
            ...
            <div id="vmt-view-cat3" class="vmt-view-panel">
                ...
                <div style="padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; margin-bottom: 10px;">
                    ...
                    <button class="glow-btn" id="btn-run-manual-khotian" style="--clr:#ff1867; width: 100%; justify-content: center; margin-bottom: 0;">
                        <span class="btn-content">🚀 অটো এন্ট্রি শুরু করুন</span><i></i>
                    </button>
                </div>
            </div>
        </div>
        ...
        if(manBtn) {
            manBtn.onclick = async () => {
                ...
                try {
                    if (window._vmtTools.t11) { await window._vmtTools.t11(upa, raw); }
                } catch(e) {
                    console.error(e); alert("সমস্যা হয়েছে: " + e.message);
                }
        ...
        document.getElementById('vmt-logout').onclick = async () => {
            if(confirm('আপনি কি লগআউট করতে চান?')) { 
                clearAuthData(); window.postMessage({ type: 'VMT_LOGOUT' }, '*'); tk.remove(); location.reload();
            }
        };
        ...
        async function checkPass() {
            ...
            try {
                ...
                if (data.success === true) {
                    ...
                    saveAuthData(btoa(userId), flagToSave); savedAuth = btoa(userId); tFlag = flagToSave;
    ...
    console.log("✅ VUMI PRO: Initialization Complete.");
        ...
        console.error("VUMI PRO CRITICAL ERROR:", err);
    }
})();
