javascript:(async function() {
    // আগের কোনো প্যানেল বা লগিন স্ক্রিন আটকে থাকলে সেটা ক্লিন করে দেবে
    let oldTk = document.getElementById('vumi-master-toolkit');
    if (oldTk) oldTk.remove();
    let oldOverlay = document.getElementById('vmt-pass-overlay');
    if (oldOverlay) oldOverlay.remove();
    let old404 = document.getElementById('vmt-404-overlay');
    if (old404) old404.remove();

    // সরাসরি টুলকিট লোড (কোনো লগিনের প্যারা নাই)
    loadToolkit();
    ...
    window._vmtKeyHandler = window._vmtKeyHandler || function(e) {
        ...
        if (e.altKey && !e.ctrlKey && !e.shiftKey) {
            ...
            else if (k.toLowerCase() === 'm' || c === 'KeyM') toolNum = 't11';
    ...
    function loadToolkit() {
        ...
        <div class="vmt-body" style="min-height: 260px;">
            ...
            <div id="vmt-view-cat1" class="vmt-view-panel">
                ...
                <div class="vmt-tool glow-btn" data-tool="t8" style="--clr:#6eff3e;"><span class="btn-content"><span class="vmt-tool-icon">📥</span> Auto Download (Pro)</span><span class="shortcut-badge">Alt+8</span><i></i></div>
            ...
            <div id="vmt-view-cat3" class="vmt-view-panel">
                <button class="vmt-back-btn">🔙 মেইন মেনু</button>
                <div class="vmt-tool glow-btn" data-tool="t11" style="--clr:#ff1867;"><span class="btn-content"><span class="vmt-tool-icon">📝</span> Manual Khotian (Auto Fill)</span><span class="shortcut-badge">Alt+M</span><i></i></div>
            </div>
        </div>
        ...
        window._vmtTools = {
            t1: function() {
                let t = { i: null, s: null };
                let st = ['খতিয়ান বক্স', 'সংরক্ষণ বাটন']; let c = 0; alert("১. খতিয়ান বক্সে ক্লিক কর\n২. সংরক্ষণ বাটনে ক্লিক কর");
                document.onclick = function(e) {
                    if (e.target.closest('#vumi-master-toolkit')) return;
                    let el = e.target; t[c === 0 ? 'i' : 's'] = el; el.style.border = "4px solid #ef4444"; c++;
                    if (c < 2) { console.log("Next: " + st[c]); } else {
                        document.onclick = null;
                        let r = prompt("রেঞ্জ দাও (যেমন: ১৭৭-২০০):");
                        if (r) {
                            ...
                            (async function() {
                                for (let i = p[0]; i <= p[1]; i++) {
                                    ...
                                    t.s.click(); console.log("Saved: " + b); await new Promise(r => setTimeout(r, 6000));
                                }
                                alert("কাম শেষ!");
                            })();
                        }
                    ...
                    try {
                        ...
                        if (el) {
                            ...
                            if (target) {
                                ...
                                if (s2 && s2.classList.contains('select2')) { s2.style.border = "4px solid #ef4444"; s2.style.borderRadius = "6px"; } else el.style.border = "4px solid #ef4444";
                                return true;
                            }
                        }
            ...
            t3: function() {
                ...
                if (typeof jQuery === 'undefined' && typeof $ === 'undefined') { alert('jQuery নেই'); return; }
                let updatedCount = 0;
                    ...
                    r.onload = async function(ev) {
                        ...
                        oL.forEach(l => {
                            if (l.includes("সাং") || l.includes("ঠিকানা")) { 
                                let a = l.replace(/(সাং|ঠিকানা)[:\s]*/, "").trim(); 
                                pn.forEach(p => { p.address = a; es.push(p); }); pn = []; 
                            }
                            else if (l.includes("পিং") || l.includes("জং") || l.includes("জঃ")) { 
                                let rel = l.trim(); 
                                if (rel.startsWith("পিং")) { rel = rel.replace(/^পিং[:\s]*/, "").trim(); } 
                        ...
                        let dgs = [], cls = [];
                        dL.forEach(l => { let p = l.split(/\s+/); if (p.length >= 2) { dgs.push(p[0]); cls.push(p[p.length - 1]); } else if (p.length === 1) { dgs.push(p[0]); cls.push(""); } });
                        let amt = [];
                        if (dgs.length > 0) {
                        ...
                        if (dgs.length > 0) {
                            function uS2(w, el, tx) {
                                if (!el || !tx) return;
                                let o = Array.from(el.options); let tg = o.find(x => x.text.trim() === tx.trim()) || o.find(x => x.text.includes(tx.trim()));
                    ...
                    r.onload = async function(ev) {
                        ...
                        ls.forEach(l => {
                            ...
                            if (m) {
                                ...
                                let n = (ana / 16) + (gonda / 320) + (kora / 1280) + (kranti / 3840) + (til / 76800); if (n > 0) ss = n.toFixed(6).replace(/0+$/, '').replace(/\.$/, '')
                            }
                            let c = l.replace(/[৵৶।॥৲৷১/০-৯0-9\s৸⁄৷৷/८]+$/, "").trim();
                            if (c !== "") {
                                if (c.includes("সাং") || c.includes("ঠিকানা")) { let ad = c.replace(/(সাং|ঠিকানা)[:\s]*/, "").trim(); ps.forEach(p => { p.address = ad; es.push(p) }); ps = []; } 
                                else if (c.includes("পিং") || c.includes("জং") || c.includes("জঃ")) { let rel = c.trim(); if (rel.startsWith("পিং")) { rel = rel.replace(/^পিং[:\s]*/, "").trim(); } for (let j = ps.length - 1; j >= 0; j--) { if (ps[j].father === "") { ps[j].father = rel; if (ss) ps[j].share = ss; } else break; } } 
                        ...
                        if (!proceed) return;
                        for (let d of es) {
                            let b = Array.from(document.querySelectorAll('button')).find(x => x.innerText.includes('লিখে এন্ট্রি'));
                            if (b) b.click(); await new Promise(r => setTimeout(r, 1800));
                            let ins = Array.from(document.querySelectorAll('input:not([type="hidden"]),textarea'));
                            let nF = ins.find(el => (el.placeholder || "").includes("নাম") && !((el.placeholder || "").includes("পিতা"))); if (nF) { nF.value = d.name; nF.dispatchEvent(new Event('input', { bubbles: true })) }
                            ...
                            let sFs = Array.from(document.querySelectorAll('input')).filter(el => (el.placeholder || "").includes("অংশ") && el.offsetParent !== null); let sF = sFs[sFs.length - 1];
                            if (sF && d.share) { sF.value = d.share; sF.dispatchEvent(new Event('input', { bubbles: true })) }
            ...
            t6: function() {
                ...
                if (!p) return; let cl = p.replace(/[০-৯]/g, d => "০১২৩৪৫৬৭৮৯".indexOf(d)).trim();
            ...
            t7: function() {
                ...
                solve(window);
                if(totalUpdated>0){ alert(totalUpdated+" টি সারির সব ঘর (দাগ, ২টা পরিমাণ ও শ্রেণী) আপডেট হয়েছে!") }else{ alert("কোনো ঘর পাওয়া যায়নি ভাই! পেজ রিফ্রেশ করে আবার চেষ্টা করুন।") }
            },
            t8: async function() {
                ...
                let range = prompt("ভাই, কত থেকে কত খতিয়ান নামাবেন?\n(যেমন: 106-110)");
                if (!range || !range.includes('-')) return alert("ভুল হয়েছে! মাঝখানে ড্যাশ (-) দিয়ে লিখুন।");
                ...
                for (let k = start; k <= end; k++) {
                    ...
                    try {
                        ...
                        searchBox.style.backgroundColor = '#ffff00'; searchBox.style.border = '3px solid red'; searchBox.style.outline = 'none'; searchBox.scrollIntoView({ behavior: 'smooth', block: 'center' }); searchBox.focus();
                        setNativeValue(searchBox, ''); await new Promise(r => setTimeout(r, 300));
                        setNativeValue(searchBox, banglaK); searchBox.dispatchEvent(new Event('change', { bubbles: true })); searchBox.dispatchEvent(new KeyboardEvent('keyup', { key: 'Enter', keyCode: 13, bubbles: true }));
                        ...
                        if (!searchBtn) { searchBtn = allBtns.find(b => { let svg = b.querySelector('svg'); if (svg && !b.innerText && !b.innerHTML.includes('refresh') && !b.innerHTML.includes('close')) return true; return false; }); }
                        if (searchBtn) { searchBtn.style.border = '3px solid blue'; searchBtn.click(); } else { searchBox.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', code: 'Enter', keyCode: 13, bubbles: true })); }
                        
                        let viewBtn = null;
                        for (let wait = 0; wait < 20; wait++) {
                            await new Promise(r => setTimeout(r, 500)); let tableRows = document.querySelectorAll('tbody tr');
                        ...
                        if (!viewBtn) continue;
                        viewBtn.click(); await new Promise(r => setTimeout(r, 3000));
                        
                        let rows = document.querySelectorAll('table tr'); let finalOutput = [];
                        rows.forEach(row => { let cols = row.querySelectorAll('td'); if (cols.length > 1) { let nameDetails = cols[0].innerText.trim(); let symbols = cols[1].innerText.trim(); if (nameDetails && nameDetails !== '১' && !nameDetails.includes('মালিক, অকৃষি')) { finalOutput.push(nameDetails + '    ' + symbols); } } });
                        let resultText = finalOutput.join('\n\n');
                        if (resultText !== '') { let bodyText = document.body.innerText; let khatianMatch = bodyText.match(/খতি[য়য়]ান\s*নং[^\d০-৯]*([০-৯0-9]+)/); let fileName = (khatianMatch ? khatianMatch[1] : 'khatian_data_' + banglaK) + '.txt'; let blob = new Blob([resultText], { type: 'text/plain;charset=utf-8' }); let link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = fileName; document.body.appendChild(link); link.click(); link.remove(); }
                        await new Promise(r => setTimeout(r, 1000));
                ...
                alert("🎉 আপনার দেওয়া রেঞ্জের সব ডাউনলোড শেষ ভাই!");
            },
            t9: function() {
                ...
                function ad(rf, cd) {
                    ...
                    d.querySelector('.cpt').onclick = () => { ad(d, { a: d.querySelector('.sa').value, g: d.querySelector('.sg').value, k: d.querySelector('.sk').value, kr: d.querySelector('.skr').value, t: d.querySelector('.st').value }); };
                    d.querySelector('.ib').onclick = () => ad(d); d.querySelector('.db').onclick = () => { d.remove(); rn(); };
                    if (rf) rf.after(d); else document.getElementById('lst').appendChild(d);
                    if (cd) { d.querySelector('.sa').value = cd.a; d.querySelector('.sg').value = cd.g; d.querySelector('.sk').value = cd.k; d.querySelector('.skr').value = cd.kr; d.querySelector('.st').value = cd.t; } rn();
                    setTimeout(() => { d.querySelector('.sa').focus(); }, 10);
                }
            ...
            t10: function() {
                ...
                if (!calcWin) { alert('আপনার ব্রাউজার পপ-আপ ব্লক করেছে! অনুগ্রহ করে এড্রেস বারের ডান পাশ থেকে পপ-আপ অ্যালাউ করে আবার চেষ্টা করুন।'); return; }
...
<head>
    ...
    <style>
        :root { --bg-color: #f4f7f6; --card-bg: #ffffff; --primary: #1e3a8a; --success: #27ae60; --danger: #e74c3c; --text-dark: #0f172a; --border-color: #e2e8f0; }
        ...
        .container { width: 100%; max-width: 1100px; background: var(--card-bg); border-radius: 12px; box-shadow: 0 8px 20px rgba(0,0,0,0.08); padding: 25px; overflow-x: auto; border-top: 4px solid #1e3a8a; }
        h2 { text-align: center; color: var(--primary); margin-top: 0; border-bottom: 2px solid #f1f5f9; padding-bottom: 15px; }
        ...
        th { background: #f8fafc; color: var(--text-dark); font-size: 14px; }
        ...
        .copy-actions .btn { flex: 1; min-width: 200px; padding: 12px; }
...
<div class="container">
    ...
    <div class="footer-total">
        ...
        <div class="copy-actions">
            ...
            <button id="btn-copy-shreni" class="btn" style="color: #3b82f6;" onclick="copyAllShreni()">সব শ্রেণি কপি</button>
        </div>
    </div>
</div>
<script>
    function convertToEnglish(str) { if (!str) return ""; const bngToEng = {'০':'0', '১':'1', '২':'2', '৩':'3', '৪':'4', '৫':'5', '৬':'6', '৭':'7', '৮':'8', '৯':'9'}; return str.toString().replace(/[০-৯]/g, match => bngToEng[match]); }
    function toBengaliNumber(num) { if(num === undefined || num === null) return "০.০০"; let str = num.toString(); let isNegative = false; if (str.startsWith('-')) { isNegative = true; str = str.substring(1); } const bngDigits = ['০','১','২','৩','৪','৫','৬','৭','৮','৯']; let result = str.split('').map(d => bngDigits[d] || d).join(''); return isNegative ? '-' + result : result; }
    function updateSerialNumbers() { const rows = document.querySelectorAll('#table-body tr'); rows.forEach((row, index) => { row.querySelector('.serial-no').innerText = toBengaliNumber(index + 1); }); }
    function handleVerticalEnter(e, inputElement, className) { if (e.key === 'Enter') { e.preventDefault(); const currentRow = inputElement.closest('tr'); let nextRow = currentRow.nextElementSibling; if (!nextRow) { addRow(); nextRow = currentRow.nextElementSibling; } if (nextRow) { const nextInput = nextRow.querySelector('.' + className); if (nextInput) nextInput.focus(); } } }
    ...
    function applyQuickDeduction() {
        ...
        rows.forEach(row => {
            ...
            if (rowDag === targetDag) { found = true; const deductionInputs = row.querySelectorAll('.deduction-input'); let targetInput = Array.from(deductionInputs).find(input => input.value.trim() === ""); if (!targetInput) { const btn = row.querySelector('.add-box-btn'); targetInput = addDeductionBox(btn); } targetInput.value = toBengaliNumber(amount); checkAndAddExtraBox(row); calculateRow(row.querySelector('.dag-input')); row.style.transition = "background-color 0.5s"; row.style.backgroundColor = "#d4edda"; setTimeout(() => row.style.backgroundColor = "", 1500); }
        });
        if (found) { msgSpan.innerText = "✓ যোগ হয়েছে!"; msgSpan.style.color = "#27ae60"; dagInput.value = ""; amountInput.value = ""; dagInput.focus(); } else { msgSpan.innerText = "দাগ নং খুঁজে পাওয়া যায়নি!"; msgSpan.style.color = "#e74c3c"; } setTimeout(() => msgSpan.innerText = "", 2000);
    }
    function handleShreniShortcut(input) { const val = input.value.trim(); const shortcuts = {'1': 'বাড়ী', '১': 'বাড়ী', '2': 'বোরো', '২': 'বোরো', '3': 'বোর', '৩': 'বোর', '4': 'খামা', '৪': 'খামা', '5': 'বর্ষা', '৫': 'বর্ষা', '6': 'চালা', '৬': 'চালা', '7': 'সাইল', '৭': 'সাইল', '8': 'কবরস্থান', '৮': 'কবরস্থান', '9': 'পুকুর', '৯': 'পুকুর'}; if (shortcuts[val]) input.value = shortcuts[val]; }
    function addRow(dag = "", shreni = "", total = "") {
        ...
        tr.innerHTML = \`
            ...
            <td><div style="display: flex; align-items: center; justify-content: center; gap: 5px;"><span class="remaining-land">০.০০</span><button class="copy-btn-sm" onclick="copyRemaining(this)" style="padding:4px 8px; font-size:11px;">কপি</button></div></td>
    ...
    function addDeductionBox(btn) { const wrapper = btn.closest('.deduction-wrapper'); const input = document.createElement('input'); input.type = 'text'; input.placeholder = 'জমি'; input.className = 'deduction-input'; input.oninput = function() { calculateRow(this); }; input.onkeypress = function(e) { handleDeductionEnter(e, this); }; wrapper.insertBefore(input, btn); return input; }
    function checkAndAddExtraBox(row) { const inputs = row.querySelectorAll('.deduction-input'); const filledCount = Array.from(inputs).filter(i => i.value.trim() !== "").length; if (filledCount >= inputs.length - 1) { addDeductionBox(row.querySelector('.add-box-btn')); } }
    function calculateRow(el) { const row = el.closest('tr'); checkAndAddExtraBox(row); let total = parseFloat(convertToEnglish(row.querySelector('.total-land').value)) || 0; let deducted = 0; row.querySelectorAll('.deduction-input').forEach(i => { deducted += parseFloat(convertToEnglish(i.value)) || 0; }); let rem = total - deducted; const remDisplay = row.querySelector('.remaining-land'); remDisplay.innerText = toBengaliNumber(rem.toFixed(2)); if (rem < 0) { remDisplay.style.color = "var(--danger)"; } else { remDisplay.style.color = "var(--success)"; } calculateGrandTotal(); }
    function calculateGrandTotal() { let grand = 0; document.querySelectorAll('.remaining-land').forEach(el => { grand += parseFloat(convertToEnglish(el.innerText)) || 0; }); const gtDisplay = document.getElementById('grand-total'); gtDisplay.innerText = toBengaliNumber(grand.toFixed(2)); if (grand < 0) { gtDisplay.style.color = "var(--danger)"; } else { gtDisplay.style.color = "#1e3a8a"; } }
    ...
    function showCopyFeedback(btnId, originalText) { const btn = document.getElementById(btnId); btn.innerText = "✓ কপি হয়েছে"; btn.style.color = "#27ae60"; setTimeout(() => { btn.innerText = originalText; btn.style.color = ""; }, 1500); }
    function copyRemaining(btn) { let val = parseFloat(convertToEnglish(btn.previousElementSibling.innerText)) || 0; let copyVal = val < 0 ? 0 : val; navigator.clipboard.writeText(toBengaliNumber(copyVal.toFixed(2))).then(() => { btn.innerText = "✓"; setTimeout(() => btn.innerText = "কপি", 1000); }); }
    function copyAllDags() { const vals = Array.from(document.querySelectorAll('.dag-input')).map(i => i.value).filter(v => v); navigator.clipboard.writeText(vals.join(', ')).then(() => { showCopyFeedback('btn-copy-dag', 'সব দাগ নং কপি'); }); }
    function copyAllShreni() { const vals = Array.from(document.querySelectorAll('.shreni-input')).map(i => i.value).filter(v => v); navigator.clipboard.writeText(vals.join(', ')).then(() => { showCopyFeedback('btn-copy-shreni', 'সব শ্রেণি কপি'); }); }
    function copyAllRemaining() { const vals = Array.from(document.querySelectorAll('.remaining-land')).map(el => { let val = parseFloat(convertToEnglish(el.innerText)) || 0; let finalVal = val < 0 ? 0 : val; return toBengaliNumber(finalVal.toFixed(2)); }); navigator.clipboard.writeText(vals.join(', ')).then(() => { showCopyFeedback('btn-copy-land', 'সব অবশিষ্ট জমি কপি'); }); }
    document.addEventListener('keydown', function(e) { const active = document.activeElement; if (!active || active.tagName !== 'INPUT' || !active.closest('#table-body')) return; const row = active.closest('tr'); const rows = Array.from(document.querySelectorAll('#table-body tr')); const rowIndex = rows.indexOf(row); const inputs = Array.from(row.querySelectorAll('input[type="text"]')); const colIndex = inputs.indexOf(active); if (e.key === 'ArrowUp' && rowIndex > 0) rows[rowIndex-1].querySelectorAll('input[type="text"]')[colIndex]?.focus(); else if (e.key === 'ArrowDown' && rowIndex < rows.length - 1) rows[rowIndex+1].querySelectorAll('input[type="text"]')[colIndex]?.focus(); else if (e.key === 'ArrowRight' && active.selectionStart === active.value.length) inputs[colIndex + 1]?.focus(); else if (e.key === 'ArrowLeft' && active.selectionEnd === 0) inputs[colIndex - 1]?.focus(); });
    function importDags() { let input = prompt("তালিকা পেস্ট করুন (যেমন: ৭৩৪ — বোরো — ১৬)"); if (input) { input.split('\\n').filter(l => l.trim()).forEach(line => { let parts = line.split(/[—\\-]+/).map(s => s.trim()); addRow(parts[0] || "", parts[1] || "", parts[2] || ""); }); } }
    window.onload = () => { if (document.querySelectorAll('#table-body tr').length === 0) addRow(); };
            ...
            t11: async function() {
                const upazila = prompt("উপজেলা লিখুন:", "সদর") || "";
                const FC = (prompt("খতিয়ান ফাইলের পুরো টেক্সট পেস্ট করুন:", "") || "").normalize("NFC");
                ...
                await (async function ownerPart(upazila, FC) {
                    ...
                    async function setId(id, v, tries = 5, gap = 450) {
                        ...
                        for (let i = 0; i < tries; i++) {
                            ...
                            if (el && !el.disabled) { reactSet(el, v); await sleep(gap); const el2 = document.getElementById(id); if (el2 && norm(el2.value) === norm(v)) return true; } else { await sleep(gap); }
                        } return false;
                    }
                    function setByLabel(labelText, v) {
                        ...
                        for (const p of Array.from(document.querySelectorAll("p, label"))) {
                            if (norm(p.textContent).startsWith(want)) {
                                ...
                                for (let i = 0; i < 4 && c; i++) { const inp = c.querySelector("input:not([type=hidden]):not([type=radio]), textarea"); if (inp) { reactSet(inp, v); return true; } c = c.parentElement; }
                            }
                        } return false;
                    }
                    function clickRadio(nameAttr, optionText) {
                        ...
                        for (const r of Array.from(document.querySelectorAll(`input[type=radio][name="${nameAttr}"]`))) {
                            const txt = norm((r.closest("label") || r.parentElement || {}).textContent); if (txt === want) { r.click(); return true; }
                        } return false;
                    }
                    function clickButton(text) {
                        const want = norm(text);
                        for (const b of Array.from(document.querySelectorAll("button, [role=button]"))) {
                            if (norm(b.textContent).includes(want)) { if (b.disabled) return false; b.click(); return true; }
                        } return false;
                    }
                    async function selectMouza() {
                        ...
                        if (input) input.focus(); if (popBtn) popBtn.click(); await sleep(1300);
                        const opts = Array.from(document.querySelectorAll('li[role="option"]')); if (opts.length === 1) { opts[0].click(); return true; } return false;
                    }
                    ...
                    function splitNumbered(value) { if (!value) return []; if (!/[০-৯0-9]+\s*[.)]/.test(value)) return [value.trim()]; const parts = value.split(/\s*[,;]?\s*[০-৯0-9]+\s*[.)]\s*/).map(s => s.trim()).filter(Boolean); return parts.length ? parts : [value.trim()]; }
                    function parseGuardianBlock(block) {
                        let sami = "", pita = "", mata = "";
                        for (const s of (block || "").split(",").map(x => x.trim())) { if (/^স্বামী/.test(s)) sami = s.replace(/^স্বামী\s*[-–—:：ঃ]?\s*/, "").trim(); else if (/^পিতা/.test(s)) pita = s.replace(/^পিতা\s*[-–—:：ঃ]?\s*/, "").trim(); else if (/^মাতা/.test(s)) mata = s.replace(/^মাতা\s*[-–—:：ঃ]?\s*/, "").trim(); }
                    ...
                    await setId("DIVISION_NAME", "ঢাকা"); await setId("DISTRICT_NAME", "নারায়ণগঞ্জ"); await setId("UPAZILA_NAME", upazila); await sleep(1200); await selectMouza(); await sleep(900); await setId("NAMJARI_CASE_NO", mamlaNo);
                    if (mamlaDate) setByLabel("মামলার তারিখ", bn2en(mamlaDate)); clickRadio("HAS_DHARA", "না"); await setId("KHATIAN_NO", khatianNo);

                    for (let i = 0; i < owners.length; i++) {
                        const o = owners[i]; clickRadio("IDENTITY_TYPE", "নাই"); await sleep(300); await setId("OWNER_NAME", o.name); if (o.gName) clickRadio("GUARDIAN_TYPE", o.gType); await setId("GUARDIAN", o.gName); await setId("MOTHER_NAME", o.mother); await setId("OWNER_AREA", ownerShare); await setId("OWNER_GROUP", o.group); await setId("OWNER_ADDRESS", address); await sleep(300); clickButton("মালিক যোগ করুন"); await sleep(1600);
                    }
                    clickButton("মালিক এন্ট্রি সমাপ্ত করুন");
                })(upazila, FC);

                await new Promise((r) => setTimeout(r, 1500));

                await (async function dagPart(file) {
                    if (!file) return;
                    const sleep = (ms) => new Promise((r) => setTimeout(r, ms)); const norm = (s) => (s || "").normalize("NFC").replace(/\s+/g, " ").replace(/\*/g, "").trim();
                    const bn = "০১২৩৪৫৬৭৮৯"; const bn2en = (s) => (s || "").replace(/[০-৯]/g, (d) => bn.indexOf(d)); const en2bn = (s) => String(s).replace(/[0-9]/g, (d) => bn[+d]);
                    
                    const reactSet = (el, val) => { const proto = el.tagName === "TEXTAREA" ? HTMLTextAreaElement.prototype : HTMLInputElement.prototype; Object.getOwnPropertyDescriptor(proto, "value").set.call(el, val); el.dispatchEvent(new Event("input", { bubbles: true })); el.dispatchEvent(new Event("change", { bubbles: true })); el.dispatchEvent(new Event("blur", { bubbles: true })); };
                    async function setById(id, value) { if (value === "" || value == null) return; const el = document.getElementById(id); if (!el) return; for (let t = 0; t < 5; t++) { reactSet(el, value); await sleep(350); if (norm(el.value) === norm(value)) break; } }
                    async function setRemarks(value) {
                        ...
                        if (!el) return; for (let t = 0; t < 5; t++) { reactSet(el, value); await sleep(350); if (norm(el.value) === norm(value)) break; }
                    }
                    const labelInputs = (labelText) => { const want = norm(labelText), out = []; for (const inp of document.querySelectorAll("input,textarea")) { let lbl = "", c = inp.parentElement; for (let i = 0; i < 6 && c; i++) { const p = c.querySelector("p,label"); if (p && norm(p.textContent)) { lbl = norm(p.textContent); break; } c = c.parentElement; } if (lbl.startsWith(want)) out.push(inp); } return out; };
                    async function pickAuto(inputEl, optionText) { if (!inputEl) return; inputEl.focus(); reactSet(inputEl, optionText); await sleep(700); let opts = Array.from(document.querySelectorAll('li[role="option"]')); if (!opts.length) { const root = inputEl.closest(".MuiAutocomplete-root"); const pop = root && root.querySelector(".MuiAutocomplete-popupIndicator"); if (pop) pop.click(); await sleep(700); opts = Array.from(document.querySelectorAll('li[role="option"]')); } const m = norm(optionText); const t = opts.find((o) => norm(o.textContent) === m) || opts.find((o) => norm(o.textContent).includes(m)) || opts.find((o) => m.includes(norm(o.textContent))); if (!t) return; t.click(); await sleep(350); }
                    async function pickSelect(labelStart, wantSubstr) { let trigger = null; for (const sel of document.querySelectorAll(".MuiSelect-select,[role='combobox']")) { let lbl = "", c = sel.parentElement; for (let i = 0; i < 6 && c; i++) { const p = c.querySelector("p,label"); if (p && norm(p.textContent)) { lbl = norm(p.textContent); break; } c = c.parentElement; } if (lbl.startsWith(norm(labelStart))) { trigger = sel; break; } } if (!trigger) return; trigger.focus(); trigger.dispatchEvent(new MouseEvent("mousedown", { bubbles: true })); trigger.click(); await sleep(800); let opts = Array.from(document.querySelectorAll('.MuiMenu-list li,.MuiMenuItem-root,[role="option"],[role="menuitem"]')); opts = [...new Set(opts)].filter((o) => norm(o.textContent)); const t = opts.find((o) => norm(o.textContent).includes(norm(wantSubstr))); if (!t) { document.body.click(); return; } t.dispatchEvent(new MouseEvent("mousedown", { bubbles: true })); t.click(); await sleep(350); }
                    const clickButton = (text) => { const want = norm(text); const b = Array.from(document.querySelectorAll("button")).find((x) => !x.disabled && norm(x.textContent).includes(want)); if (!b) return false; b.click(); return true; };
                    
                    const softRe = (label) => label.normalize("NFC").replace(/[নণ]/g, "[নণ]").replace(/[িী]/g, "[িী]").replace(/ /g, "\\s*");
                    const grab = (label) => { const m = file.match(new RegExp(softRe(label) + "\\s*[:：ঃ]\\s*([\\s\\S]*?)(?:\\s{2,}|\\n|$)")); return m ? m[1].trim() : ""; };
                    ...
                    const share = (atra, total) => { const a = parseFloat(bn2en(String(atra))), t = parseFloat(bn2en(String(total))); if (!a || !t) return ""; return en2bn((a / t).toFixed(5).replace(/(\.\d*?)0+$/, "$1").replace(/\.$/, "")); };
                    const grabMontobbo = () => { const mm = file.match(new RegExp(softRe("মন্তব্য") + "\\s*[:：ঃ]\\s*([\\s\\S]*)")); if (!mm) return ""; let v = mm[1]; const dot = v.indexOf("।"); if (dot !== -1) v = v.slice(0, dot + 1); return v.replace(/\s+/g, " ").trim(); };

                    const dags = splitField(grab("দাগ নং") || grab("দাগ")); const classToks = splitField(grab("জমির শ্রেণি") || grab("শ্রেণি") || grab("শ্রেণী")); const totalToks = splitField(grab("দাগে মোট জমির পরিমান") || grab("মোট জমির পরিমান") || grab("মোট")); const atraToks = splitField(grab("অত্র খতিয়ানের জমির পরিমান") || grab("অংশ অনুযায়ী জমির পরিমান") || grab("অত্র")); const pr = grab("অংশ"); const portionList = (pr && !pr.replace(/[\d০-৯\s.,\/;।:：ঃ-]/g, "")) ? splitField(pr).map(amtOf).filter(Boolean) : []; const classes = (classToks.length ? classToks : totalToks).map(classOf).filter(Boolean); const totals = totalToks.map(amtOf).filter(Boolean); const atras = atraToks.map(amtOf).filter(Boolean); const montobbo = grabMontobbo();
                    ...
                    for (let i = 0; i < dags.length; i++) {
                        ...
                        if (i === maxIdx) await setRemarks(montobbo); await sleep(400); clickButton("দাগ যোগ করুন"); await sleep(1600);
                    }
                    await sleep(500); clickButton("দাগ এন্ট্রি সমাপ্ত করুন");
                })(FC);
                alert("✅ ফুল প্রসেস শেষ — ভ্যালু চেক করে সেভ করো!");
            }
        };
