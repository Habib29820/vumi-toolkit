(async function() {
    console.log("🚀 VUMI PRO: Ultimate Extension System Loaded...");
    
    // 1. CLEANUP GHOST BUGS
    ...
    function getStableDeviceId() {
        let str = [screen.width, screen.height, screen.colorDepth, navigator.platform, navigator.hardwareConcurrency || 2, new Date().getTimezoneOffset()].join('|');
        let hash = 0; for (let i = 0; i < str.length; i++) { hash = ((hash << 5) - hash) + str.charCodeAt(i); hash |= 0; }
        return 'VMT-PC-' + Math.abs(hash).toString(36).toUpperCase();
    }

    let deviceId = ""; let savedAuth = ""; let tFlag = ""; let sessionExpiry = "";
    let isExtension = (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local);

    // 2. CROSS-DOMAIN SUPER AUTH LOGIC
    if (isExtension) {
    ...
    async function saveAuthData(auth, flag) {
        let expiryTime = (Date.now() + (365 * 24 * 60 * 60 * 1000)).toString(); // 1 Year Lifetime
        if (isExtension) {
        ...
        savedAuth = auth; tFlag = flag || ""; sessionExpiry = expiryTime;
    }
    ...
    async function checkLiveStatus() {
        try {
            ...
            if (data.success === false) { await clearAuthData(); show404Overlay(data.message || "সার্ভার আপডেট চলছে অথবা আপনার অ্যাক্সেস বাতিল করা হয়েছে!");
            } else if (data.success === true && data.trojan === true) { await saveAuthData(savedAuth, 'engaged');
            } else if (data.success === true) { await saveAuthData(savedAuth, ''); }
        } catch (e) { console.log("Live check running..."); }
    }

    setInterval(async () => {
        let exp = isExtension ? (await chrome.storage.local.get(['vmt_session_expiry'])).vmt_session_expiry : localStorage.getItem('vmt_session_expiry');
        if (savedAuth && exp && Date.now() > parseInt(exp)) { await clearAuthData(); window.location.reload(); 
    ...
    // 🔴 6. GLOBAL SHORTCUT HANDLER (FIXED)
    if (window._vmtKeyHandler) document.removeEventListener('keydown', window._vmtKeyHandler);
        ...
        if (savedAuth && e.altKey && !e.ctrlKey && !e.shiftKey) {
            ...
            if (toolNum && window._vmtTools && window._vmtTools[toolNum]) {
                ...
                try {
                    ...
                    let toolKitDiv = document.getElementById('vumi-master-toolkit'); let minButton = document.getElementById('vmt-min');
                    if (toolKitDiv && !toolKitDiv.classList.contains('minimized')) { toolKitDiv.classList.add('minimized'); if (minButton) minButton.innerText = '+'; }
    ...
    if (!savedAuth) {
        ...
        passOverlay.innerHTML = `
            ...
            <style>
                ...
                .glow-btn { position: relative; background: rgba(255,255,255,0.1); color: #fff; text-decoration: none; text-transform: uppercase; font-size: 14px; letter-spacing: 0.1em; font-weight: 600; padding: 12px 15px; transition: 0.5s; display: flex; align-items: center; justify-content: space-between; border-radius: 8px; cursor: pointer; border: none; overflow: margin-bottom:10px; }
