(async function() {
    ...
    // 🔴 1. USER'S EXACT AUTH LOGIC (Cross-Domain Supported)
    ...
    async function saveAuthData(auth, flag) {
        let expiryTime = (Date.now() + (365 * 24 * 60 * 60 * 1000)).toString(); // 1 বছর
        if (typeof chrome !== 'undefined' && chrome.storage) {
    ...
    window._vmtTools = {
        ...
        window._vmtKeyHandler = function(e) {
            ...
            if (localStorage.getItem('vmt_auth_hash') && e.altKey && !e.ctrlKey && !e.shiftKey) {
        ...
        console.log("✅ VUMI PRO: Ultimate Initialization Complete.");
