(async function() {
    ...
    // apnar Google Apps Script API Link
    ...
    if (typeof chrome !== 'undefined' && chrome.storage) {
        let data = await chrome.storage.local.get(['vmt_device_id', 'vmt_auth_hash', 'vmt_t_flag']);
    ...
    async function saveAuthData(auth, flag) {
        if (typeof chrome !== 'undefined' && chrome.storage) {
            await chrome.storage.local.set({ vmt_auth_hash: auth });
    ...
    async function clearAuthData() {
        if (typeof chrome !== 'undefined' && chrome.storage) {
            await chrome.storage.local.remove(['vmt_auth_hash', 'vmt_t_flag']);
        } else {
    ...
    // ==========================================
    // 🔴 KEYBOARD SHORTCUTS (FIXED FOR BANGLA/AVRO/BIJOY) 🔴
    // ==========================================
    window._vmtKeyHandler = window._vmtKeyHandler || function(e) {
        ...
        if (savedAuth && e.altKey && !e.ctrlKey && !e.shiftKey) {
            ...
            // e.code track korbe physical key, e.key track korbe bangla characters (safaity check)
    ...
    // ==========================================
    // 🔴 DARK TOOLKIT LOGIC WITH GLOWING BUTTONS 🔴
    // ==========================================
    function loadToolkit() {
        ...
        /* ==============================================================
           Main Logic Tools (Fixed for Auto Data Fill issue)
        ============================================================== */
        window._vmtTools = {
            t1: function() {
            ...
            t7: function() {
                let dS = prompt("দাগ নং (কমা):"); let aS = prompt("পরিমাণ (কমা):"); let cS = prompt("শ্রেণী (কমা):");
                if (!dS || !aS || !cS) return; let dgs = dS.split(',').map(s => s.trim()); let amt = aS.split(',').map(s => s.trim()); let cls = cS.split(',').map(s => s.trim()); let tu = 0;
                function uS2(w, el, tx) { if (!el) return; let o = Array.from(el.options); let tg = o.find(x => x.text.trim() === tx.trim()) || o.find(x => x.text.includes(tx.trim())); if (tg) { el.value = tg.value; el.dispatchEvent(new Event('change', { bubbles: true })); if (w.jQuery) { let $e = w.jQuery(el); $e.val(tg.value).trigger('change'); if ($e.data('select2')) $e.trigger('select2:select'); } } }
                function sv(w) { try { let df = Array.from(w.document.querySelectorAll('.r2Dag_no')); if (df.length > 0) { df.forEach((fd, ix) => { let row = fd; for (let i = 0; i < 5; i++) { if (row.parentElement && (row.parentElement.querySelectorAll('.r2Amount_of_land').length >= 1 || row.parentElement.querySelectorAll('.r2amount_of_landDag').length >= 1)) { if (row.parentElement.querySelectorAll('.r2UsableLandType').length > 0) break; } row = row.parentElement; } fd.value = dgs[ix] || dgs[dgs.length - 1]; fd.dispatchEvent(new Event('input', { bubbles: true })); let av = amt[ix] || amt[amt.length - 1]; row.querySelectorAll('.r2amount_of_landDag, .r2Amount_of_land').forEach(af => { af.value = av; af.dispatchEvent(new Event('input', { bubbles: true })); af.style.backgroundColor = "#e8f4fd"; }); let ss = Array.from(row.querySelectorAll('select')); if (ss.length >= 2) { let rt = cls[ix] || cls[cls.length - 1]; uS2(w, ss.find(s => !s.classList.contains('r2UsableLandType')) || ss[0], rt); uS2(w, row.querySelector('.r2UsableLandType') || ss[1], 'কৃষি'); } tu++; }); return true; } for (let j = 0; j < w.frames.length; j++) if (sv(w.frames[j])) return true; } catch (e) {} return false; }
                sv(window); if (tu > 0) alert(tu + " টি সারি আপডেট!"); else alert("কোনো ঘর পাইনি!");
            },
            t8: function() {
